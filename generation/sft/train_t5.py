...
from load_data import load_t5_data, TOKENIZER, INITIAL_DECODER_INPUT
...
def train(args, model, train_loader, dev_loader, optimizer, scheduler):
    ...
    correct_path = 'records/ground_truth_dev.pkl'
    gt_record_path = os.path.join(correct_path)
...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    with torch.no_grad():
        for batch in dev_loader:
            encoder_ids, encoder_mask, decoder_ids, decoder_targets, initial_decoder_inputs = batch
            ...
            # USE FORWARD TO CALCUALTE LOSS
            outputs = model.forward(
                input_ids=encoder_ids, attention_mask=encoder_mask,
                decoder_input_ids=decoder_ids, labels=decoder_targets)
            loss = outputs.loss
            total_loss += loss.item()

            # USE GENERATE TO PREDICT SQL
            ...
            decoded_generated_ids = TOKENIZER.batch_decode(generated_ids, skip_special_tokens=True)
            count += 1
            if count % 50 == 0:
                for i in range(3):
                    print(f"DEBUG: generated: {decoded_generated_ids[i]}")

            generated_sqls.extend(decoded_generated_ids)
            total_samples += len(encoder_ids)

    save_queries_and_records(generated_sqls, model_sql_path, model_record_path)
    dev_sql_em, dev_record_em, dev_record_f1, model_error_msgs = compute_metrics(
        gt_sql_pth, model_sql_path, gt_record_path, model_record_path) 
    print(f"DEBUG model_error_msgs: {model_error_msgs[0:3]}")
    # Compute syntax error rate
    syntax_errors = len(model_error_msgs)
    dev_error_rate = syntax_errors / total_samples if total_samples > 0 else 0
    # Compute average loss
    dev_loss = total_loss / len(dev_loader) if len(dev_loader) > 0 else 0

    log_path = os.path.join(f'records/t5_{args.experiment_name}_logs.txt')
    save_logs(log_path, dev_sql_em, dev_record_em, dev_record_f1, model_error_msgs)
...
def test_inference(args, model, test_loader, model_sql_path, model_record_path):
    ...
    with torch.no_grad():
        for batch in test_loader:
            encoder_ids, encoder_mask, _ = batch
            encoder_ids = encoder_ids.to(DEVICE)
            encoder_mask = encoder_mask.to(DEVICE)
            generated_ids = model.generate(
                decoder_start_token_id=INITIAL_DECODER_INPUT,
            ...
            # TODO: check if decoding is needed
            decoded_generated_ids = TOKENIZER.batch_decode(generated_ids, skip_special_tokens=True)
            generated_sqls.extend(decoded_generated_ids)
    save_queries_and_records(generated_sqls, model_sql_path, model_record_path)
